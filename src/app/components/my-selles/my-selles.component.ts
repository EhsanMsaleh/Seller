...
export class MySellesComponent implements OnInit {
  ...
  async wait(){
  // console.log(this.dateArr2)

  }
 /*async fffl(filterValue:string){
    this.data.filter = await filterValue
    console.log(this.data.filter)
    console.log(filterValue)
  }*/

  groupBy(list: Sales[], key){
    const map = new Map()
    list.forEach(item => {const get = key(item)
      const collection = map.get(get)
      if(!collection){
        map.set(get, [item])
